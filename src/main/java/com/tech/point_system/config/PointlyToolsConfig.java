...
public class PointlyToolsConfig {
    ...
    @Bean
    @Description("Consulta el saldo actual de puntos acumulados de un cliente en un comercio especifico usando su DNI, pais (por defecto 'Argentina') y el nombre del comercio.")
    public Function<ClientPointsRequest, ClientPointsResponse> getPointsBalance(
        ...
        return request -> {
            ...
            if (clientOpt.isEmpty()) {
                return new ClientPointsResponse(false, null, request.companyName(), 0, "No se encontro ningún cliente registrado con el DNI proporcionado.");
            }
    ...
    @Bean
    @Description("Obtiene el catalogo de premios y recompensas activos disponibles para canjear en un comercio o empresa especifico ingresando su nombre.")
    public Function<RewardsRequest, RewardsResponse> getAvailableRewards(
            CompanyRepository companyRepository,
    ...
    @Bean
    @Description("Obtiene las promociones de multiplicadores de puntos (ejemplo: 2x) vigentes en un comercio especifico ingresando su nombre.")
    public Function<PromotionRequest, PromotionResponse> getActivePromotions(
            CompanyRepository companyRepository,
