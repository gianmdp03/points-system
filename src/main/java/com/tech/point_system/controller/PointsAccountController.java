...
public class PointsAccountController {
    ...
    @PostMapping
    public ResponseEntity<PointsAccountDetailDTO> registerClientAndCreateAccount(@Valid @RequestBody PointsAccountRequestDTO dto) {
        return ResponseEntity.status(HttpStatus.CREATED).body(service.registerClientAndCreateAccount(dto));
    }
}
